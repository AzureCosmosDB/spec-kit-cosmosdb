# Recommend Determinism Report

> Empirical testing of `/speckit.cosmosdb.recommend` with vague/ambiguous inputs across 3 scenarios × 3 runs each.
...
### Key Finding
The recommend prompt's **guardrail worked perfectly** - all 3 runs correctly identified this as too ambiguous and stopped. However, the specific clarifying question varied each time, which could lead users down different paths in subsequent interactions. The safeguard behavior itself was 100% deterministic.
...
| Input Type | v1 Consistency | v2 Consistency | Recommend Consistency |
|------------|---------------|----------------|------------------|
...
**Key observations:**
1. Recommend with vague-but-directional input (~71%) falls **between** v1 and v2 explicit inputs
2. Recommend with ambiguous input (~43%) is **worse than v1** - the scaffold selection ambiguity cascades into partition key chaos
3. Recommend's guardrail for "too vague" is the most reliable behavior (~83%), but the follow-up question varies
...
### Is `/speckit.cosmosdb.recommend` deterministic enough to be a generator?
...
```
/speckit.cosmosdb.recommend (triage-only)
...
/cosmos.scaffold-{type} (generator)
    ↓ Takes EXPLICIT inputs from recommend's triage
    ↓ Generates deterministic code (v2: 95%)
```

**Action items:**
1. Modify `/speckit.cosmosdb.recommend` to output a **recommended command** instead of generating code directly
