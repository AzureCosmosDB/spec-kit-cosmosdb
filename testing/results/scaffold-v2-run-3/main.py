"""Game Leaderboard API - Mobile game score tracking with Azure Cosmos DB"""
...
async def get_global_leaderboard(request: Request, limit: int = 100):
    """Get global top 100 by score. CROSS-PARTITION: global leaderboard spans all regions."""
...
async def get_regional_leaderboard(request: Request, region: str, limit: int = 100):
    """Get regional top 100 by score filtered by region."""
    service = get_service(request)
...
async def get_player_score(request: Request, player_id: str):
    """Get single player's score and rank."""
    service = get_service(request)
